...
export const UsersController = {
  ...
  async count(req: FastifyRequest, reply: FastifyReply) {
    return reply.status(200).send(await UsersService.count());
  },

  async addHistory(req: FastifyRequest, reply: FastifyReply) {
    const body = addUserHistorySchema.parse(req.body) as AddUserHistoryDto;
  ...
  async getUserHistory(req: FastifyRequest, reply: FastifyReply) {
    const { id } = req.params as { id: string };
    const query = req.query as z.infer<typeof getUserHistorySchema>;

    const history = await UsersService.getUserHistoryById(id, query);
